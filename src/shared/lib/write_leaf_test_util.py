def write_leaf_test_util(node: dict, job_id=None) -> bool:
    import logging

    from src.shared.lib.render_test_util import render_test_util
    from src.shared.lib.test_path_for_leaf_util import test_path_for_leaf_util
    from src.shared.lib.verify_test_util import verify_test_util
    from src.shared.lib.write_file_util import write_file_util
    from src.shared.logging.event_util import event_util
    from src.shared.logging.get_logger_util import get_logger_util
    from src.pipeline.code_tree.generate_test.tests_enabled import tests_enabled

    if not tests_enabled():
        return False
    logger = get_logger_util(job_id)
    fn = node.get("function_name") or "module"
    code_path = node.get("code_path") or fn
    code = render_test_util(node)
    ok, err = verify_test_util(code, fn)
    if not ok:
        node["test_ok"] = False
        event_util(
            logger,
            "code_tree_test_verify_fail",
            level=logging.ERROR,
            function_name=fn,
            error=err,
        )
        return False
    out_path = test_path_for_leaf_util(code_path, fn)
    write_file_util(out_path, code)
    node["test_ok"] = True
    event_util(logger, "code_tree_write_test", path=out_path, function_name=fn)
    return True

def assess_job_util(root, job=None) -> tuple:
    ...
    def scaffold_issues(job_root: str) -> list:
        ...
        if (root_path / "requirement").is_dir() and (
            not (root_path / "requirement" / "requirements.txt").exists()
        ):
        ...
        if job_id and not (root_path / "src" / job_id / "__init__.py").exists():
            out.append(f"scaffold: missing src/{job_id}/__init__.py")
        return out

    def workplace_contract_issues(job_root: str) -> list:
        try:
            ...
            return [f"workplace contract check failed: {exc}"]
    ...
    if job:
        ...
        if job.get("root_path"):
            issues.extend(workplace_contract_issues(job["root_path"]))
    if issues:
        return ("incomplete", issues)
    if not is_fully_grown_util(root):

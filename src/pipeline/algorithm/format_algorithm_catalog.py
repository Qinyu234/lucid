def format_algorithm_catalog(node: dict | None=None, limit: int=6) -> str:
    from src.pipeline.algorithm.match_fixed_task import match_fixed_task
    entries = match_fixed_task(node, limit=limit) if node else []
    if not entries:
        from src.pipeline.algorithm.list_algorithms import list_algorithms
        entries = list_algorithms(limit=limit)
    lines = ['Algorithms below are ONLY valid under fixed distribution + fixed business logic.', 'Use one only when the current node matches the fixed_task regime.', '']
    for entry in entries:
        ...
        lines.append(f"  relative_advantage: {adv.get('metric', '')} vs {adv.get('baseline', '')}")
        if adv.get('margin'):

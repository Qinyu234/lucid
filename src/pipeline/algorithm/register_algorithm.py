def register_algorithm(entry: dict):
    ...
    profile = {'profile_id': profile_id, 'distribution': fixed.get('distribution', ''), 'business': fixed.get('business', ''), 'constraints': fixed.get('constraints') or []}
    index['profiles'][profile_id] = profile
    index['algorithms'][fn] = {'module': entry.get('module', f'algorithm.{fn}'), 'function_name': fn, 'category': entry.get('category', 'general'), 'semantic': entry.get('semantic', ''), 'io': normalize_io(entry.get('io')), 'fixed_task': profile, 'relative_advantage': entry.get('relative_advantage') or {}, 'complexity': entry.get('complexity') or {}, 'references': entry.get('references') or []}

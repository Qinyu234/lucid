def match_fixed_task(node: dict, limit: int=6) -> list:
    ...
    def tokens(text: str) -> set:
        if not text:
            return set()
        return set(re.findall('[a-zA-Z0-9_]{3,}', text.lower()))
    ...
    for entry in list_algorithms(limit=100):
        ...
        if not fixed.get('profile_id') or not adv.get('baseline'):
            continue
        hay = ' '.join([fixed.get('distribution', ''), fixed.get('business', ''), ' '.join(fixed.get('constraints') or []), entry.get('semantic', ''), entry.get('category', '')])

...
def pipeline(job):
    import logging
    from src.pipeline.errors.exceptions import PipelineError
    from src.pipeline.errors.error_packet import error_packet
    from src.shared.persist_io_registry import persist_io_registry
    from src.shared.validate_node import validate_node
    job_id = job.get('id', 'unknown')
    logger = get_logger(job_id)
    root = None
    try:
        log_event(logger, 'pipeline_start', job_id=job_id, goal=job.get('goal'))
        if feature_enabled('algorithm_registry_on_start'):
            from src.pipeline.algorithm.ensure_layout import ensure_layout
            ensure_layout()
        root = seed(job)
        ...
        log_event(logger, 'pipeline_finish', status=status, issues=issues if issues else None)
        return {'job_id': job_id, 'status': status, 'issues': issues, 'tree': root, 'tree_path': str(job.get('root_path', '')) + '/tree/latest.json'}
